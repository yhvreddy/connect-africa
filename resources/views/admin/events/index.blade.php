...
<div class="container-fluid">
                    ...
                    <div class="card widget-1">
                        <div class="card-body">
                            <div class="widget-content">
                                <div>
                                    <h4>{{$events->count()}}</h4><span class="f-light">Total Events</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="table-responsive">
                            <table class="display" id="eventFetchDataToTable">
                                <thead>
                                    <tr>
                                        <th>Sno</th>
                                        <th>Poster Image</th>
    ...
    <input type="hidden" value="{{route('default.event.fetch.data.ajax')}}" id="eventFetchListDataUrl" />

    @if(isset(app('truFlix')->getSessionUser()->role->slug) && app('truFlix')->getSessionUser()->role->slug === 'zq')
        <input type="hidden" value="false" id="isRequestAdmin" />
    @else
        <input type="hidden" value="{{ route('default.event.update.status', ['eventId' => ':eventId', 'action' => ':action']) }}" id="updateStatusUrl" />
...
@section('script')
    ...
    @if(isset(app('truFlix')->getSessionUser()->role->slug) && app('truFlix')->getSessionUser()->role->slug === 'zq')
        <script src="{{ asset('assets/src/js/events/zq_script.js') }}"></script>
    @else
        <script src="{{ asset('assets/src/js/events/script.js') }}"></script>
    @endif
@endsection
