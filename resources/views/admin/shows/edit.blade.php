...
@php
	...
	$oneOption = $show->getAdditionalDataByType('show_wp_one')->pluck('em_id')->toArray();
	$twoOption = $show->getAdditionalDataByType('show_wp_two')->pluck('em_id')->toArray();
	$threeOption = $show->getAdditionalDataByType('show_wp_three')->pluck('em_id')->toArray();
	$freeOption = $show->getAdditionalDataByType('show_free_option')->pluck('em_id')->toArray();
	$savedVideos = $show->getAdditionalDataByType('shows_video')->get();
	$categorizeIds = $show->getAssignedCategorizedList('shows')->pluck('categorize_id')->toArray();
@endphp
		...
		<div class="col-sm-12">
			<div class="card">
				<div class="card-header">
					<h5 class="title">Edit Show</h5>
				</div>
				<form class="form theme-form needs-validation" action="{{route('default.shows.update', $show->id)}}" method="POST" novalidate="" enctype="multipart/form-data">
					@method('PUT')
					@csrf
					<div class="card-body">
						<div class="row">
							...
							<div class="col">
								<div class="mb-3 row">
									...
									<div class="col-sm-9">
										<div class="row">
											<div class="{{(!empty($show->poster_image) && Storage::exists($show->poster_image))?'col-11':'col-12' }}">
												<input class="form-control" type="file" accept=".png, .jpg, .jpeg" name="poster_image" {{(!empty($show->poster_image) && Storage::exists($show->poster_image))?:'required' }}  />
											</div>
											@if(!empty($show->poster_image) && Storage::exists($show->poster_image))
												<div class="col-1 pt-1">
													<a href="{{asset(Storage::url($show->poster_image))}}" target="_blank">
														<img src="{{asset(Storage::url($show->poster_image))}}" style="width:38px;" alt="{{$show->title}}" />
													</a>
												</div>
											@endif
										</div>
								...
								<div class="mb-3 row">
									...
									<div class="col-sm-9">
										<input class="form-control" type="text" required="" value="{{$show->title}}" name="title" value="{{old('title')}}" />
										@if($errors->has('title'))
									...
									<div class="col-sm-9">
										<select class="form-control js-example-basic-single_" required="" name="year">
											...
											@foreach ($years as $year)
												<option {{$show->year != $year?'':'selected'}} value="{{$year}}">{{$year}}</option>
											@endforeach
										</select>
										@if($errors->has('year'))
									...
									<div class="col-sm-9">
										<select class="form-control js-example-basic-single_" required="" name="rating">
											...
											@foreach ($ratedList as $rated)
												<option value="{{$rated}}" {{$show->rating != $rated?:'selected'}}>{{$rated}}</option>
											@endforeach
										</select>
								...
								<div class="mb-3 row">
									...
									<div class="col-sm-9">
										<textarea class="form-control" name="description" rows="5" required="">{{$show->description}}</textarea>
									...
									<div class="col-sm-9">
										<select class="form-control js-example-basic-single_" required="" name="imdb_score">
											...
											@foreach ($ratings as $rating)
												<option value="{{$rating}}" {{$show->imdb_score != $rating?:'selected'}}>{{$rating}}</option>
											@endforeach
										</select>
								...
								<div class="mb-3 row">
									...
									<div class="col-sm-9">
										<select class="form-control js-example-basic-single_" required="" name="truflix_score">
											...
											@foreach ($ratings as $rating)
												<option value="{{$rating}}" {{$show->truflix_score != $rating?:'selected'}}>{{$rating}}</option>
											@endforeach
										</select>
								...
								{{-- <div class="mb-3 row">
									...
									<div class="col-sm-9">
										<select class="form-control js-example-basic-single_" required="" name="google_score">
											...
											@foreach ($ratings as $rating)
												<option value="{{$rating}}" {{$show->google_score != $rating?:'selected'}}>{{$rating}}</option>
											@endforeach
										</select>
															...
															<div class="row">
																<div class="col-12">
																	<div class="row">
																		...
																		@if(!empty($savedVideo->image) && Storage::exists($savedVideo->image))
																			<div class="col-1 pt-1">
																				<a href="{{asset(Storage::url($savedVideo->image))}}" target="_blank">
																					<img src="{{asset(Storage::url($savedVideo->image))}}" style="width:38px;height: 40px;" alt="{{$show->title}}" />
																				</a>
																			</div>
																		@endif
																	</div>
																...
																<div class="col-12 mt-2">
																	<input class="form-control" type="url" placeholder="video source link" name="update_videos[{{$key}}][link]" value="{{$savedVideo->url}}" />
																</div>
															</div>
														</div>
														<div class="col-2">
															<a href="javascript:void(0);" data-delete_link="{{route('default.shows.video.delete', ['video'=>$savedVideo->id])}}" data-id="{{$savedVideo->id}}" class="delete_uploaded_video btn btn-danger"><i class="fa fa-trash-o"></i></a>
														</div>
													</div>
												@endforeach
