...
class MoviesController extends Controller
{
     ...
     * @OA\Get(
     *   tags={"Movies"},
     *   path="/api/v1/entertainment/movies",
    ...
    public function index(Request $request)
    {
        ...
        $movies = $this->entertainmentRepository
                    ->where('entertainments.category_id', $category->id)
                    ->where('entertainments.is_active', 1);

        if(isset($request->search) && !empty($request->search)){
            $movies->where('entertainments.title', 'LIKE', '%'.$request->search.'%');
        }

        if(isset($request->year) && !empty($request->year)){
            $movies->orWhere('entertainments.year', $request->year);
        }

        if(isset($request->genres) && !empty($request->genres)){
            $movies->orWhere(function($query) use ($request) {
                $query->leftJoin(
        ...
        $listOfMovies = $movies->orderBy('id', 'DESC')
        ->paginate(30);

        return $this->success('Movies List', new MoviesCollection($listOfMovies));
    }
    ...
    /**
     * @OA\GET(
     *     path="/api/v1/entertainment/movies/{movie}",
     *     tags={"Movies"},
    ...
    public function show(Entertainment $movie)
    {
        ...
        return $this->success('Movies Details', new MoviesResource($movie));
    }
    ...
    public function categorizedList(Request $request){
        ...
        foreach($categorizes as $key => $categorize){
            $movies =   $this->entertainmentRepository
                        ...
                        ->where(['categorize_assigned_list.categorize_id' => $categorize->id, 'categorize_assigned_list.type' => 'movies'])->limit(10)->get();
            $categorize->items = new MoviesCollection($movies);
        }
